...
- [x] Mobile-friendly UI (verified with a real headless browser, not guessed — see note below)
- [ ] Full terminal emulation improvements (multiplexing, session recording) — **not attempted, deliberately**: this file's own "Out of Scope for MVP" section below explicitly excludes these, so implementing them would contradict a boundary the project already set rather than complete a stretch goal
...
**Mobile-friendly UI — exit criteria verified with a real browser, not guessed:** the previous note above (and DOCS.md before this pass) explicitly said no headless browser was available and mobile responsiveness was "whatever falls out of Tailwind's defaults," not a deliberately verified experience. That changed for this item: Playwright + a fallback headless Chromium build were installed into a throwaway venv (not a project dependency — this was one-off verification tooling, not something committed) and driven against the real running dev stack at 375×667 (iPhone SE-class) and spot-checked at 320×568 (classic minimum) and 1280×800 (desktop, to confirm no regression).

This caught three real, previously-undetected layout bugs, all now fixed:
1. **Dashboard/History/Admin headers overflowed horizontally on every page** — `AppBar.Toolbar`'s `grid-cols-[auto_1fr_auto]` plus `AppBar.Trail`'s un-wrapped flex row doesn't shrink below its content's natural width in a CSS grid `auto` track, so the History/Users/username/Log out cluster pushed the whole page 69px wider than the viewport (confirmed via `document.documentElement.scrollWidth` vs `clientWidth`, not eyeballing). Fixed in `Dashboard.tsx`, `ConnectionHistory.tsx`, and `AdminUsers.tsx` by making the toolbar stack to a single column below the `sm:` breakpoint (`grid-cols-1 sm:grid-cols-[auto_1fr_auto]`) and letting the Trail wrap (`flex-wrap justify-end`).
2. **The admin users list overflowed with real-length emails** — `AdminUsers.tsx`'s row was `flex items-center justify-between` with no `min-w-0` on the text column; an email like `mobiletest1786900134@example.com` has no hyphen/space for the browser to break on, so it forced the row (and page) 52px wider than the viewport. Fixed with `min-w-0` + `break-words` on the text column, and the row now stacks to a column below `sm:` so actions don't get squeezed next to long text.
3. **`ConnectionList`'s grid item didn't shrink for unbroken hostnames** — this is the CSS Grid min-content-sizing gotcha: `<ul className="grid ...">`'s `<li>` grid items default to `min-width: auto`, so even with `break-words` on the *nested* text, the grid item itself refused to shrink below an unbroken string's full width (confirmed concretely: a 51-character no-hyphen hostname pushed `scrollWidth` to 783px against a 375px viewport). Fixed by adding `min-w-0` to the `<li>` itself, not just its children — `break-words` on descendants is necessary but not sufficient inside a grid/flex item without that.

A fourth issue was cosmetic rather than an overflow — `TerminalPage.tsx`'s status header used `items-center` on a `justify-between` row, so a long wrapped error message (e.g. `"unable to reach host: [Errno None] Unable to connect to port 9 on 127.0.0.1"`) vertically centered the Back/Reconnect buttons against a 4-line-tall error block, reading as cramped/overlapping at a glance even though nothing was actually obscured (verified via exact bounding-box coordinates, not just the screenshot). Fixed with `items-start`.

Every fix was verified by *re-running* the same automated check (`document.documentElement.scrollWidth <= clientWidth`) after the change, across: home (logged out), login, register, dashboard (empty/one connection/two connections/tag-filtered/long-unbroken-content), both connection-form auth modes (password and SSH-key textarea), history (empty/with a real failed entry), admin (denied screen for non-admin, real user list for the actual admin), and the terminal page in all four states (connecting, a real live SSH session with real typed/echoed output via a disposable `linuxserver/openssh-server` container, closed, and a real connection error) — all at 375×667, spot-checked at 320×568 and 1280×800. `pnpm run typecheck` / `pnpm run test` (13/13, no regressions) / `pnpm run build` re-verified both locally and via `podman compose run --rm frontend ...` against a freshly built image. `DOCS.md` §11/§12 updated to drop the "not a deliberately verified mobile experience" caveat and remove mobile-friendly UI from Future Improvements.
